    ...
    return (
        ...
        >
            <div className="w-full h-full max-w-[1200px] flex flex-col mx-auto px-2 xs:px-4 py-5 xs:py-7 text-sm xl:pt-12 xl:pb-16 xl:text-lg">
                ...
                <p className="text-sm xl:text-lg drop-shadow-[0_4px_4px_rgba(0,0,0,0.25)] mb-3">{t?.spb?.p1}</p>
                <p className="text-sm xl:text-lg drop-shadow-[0_4px_4px_rgba(0,0,0,0.25)] mb-3">{t?.spb?.p2}</p>
                <div className="flex gap-5 xl:gap-10 justify-center py-4 xl:pt-20 flex-wrap">
                    {Object.keys(t?.spb?.themes || {}).map((theme, index) => (
                        <button
                            key={index}
            ...
            {selectedTheme && (
                <Popup
                    theme={selectedTheme}
                    translations={t?.spb?.themes?.[selectedTheme]}
